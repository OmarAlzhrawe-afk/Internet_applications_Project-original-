...
class AuthController extends Controller
{
    ...
    public function getAllNotifications()
    {
        ...
        $unreadNotifications = $user->unreadNotifications->map(function ($notification) {
            return [
                ...
                'notification_title' => $notification->data['title'] ?? "no title",
                'notification_message' => $notification->data['message'] ?? "no message",
                'notification_time' => $notification->data['time'] ?? $notification->created_at->format('Y-m-d H:i:s'),
                'notification_is_read' => $notification->read_at !== null,
            ];
        });
        return sendResponse($unreadNotifications, 200, "Your Notifications", false);
    }
    public function markAsRead($id)
    {
        ...
        if ($notification) {
            ...
            return sendResponse(null, 200, "Marking Notification As Read Done", false);
        } else {
            return sendResponse(null, 404, "Notification Not Found", true);
        }
    }
}
